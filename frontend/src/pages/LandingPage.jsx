    ...
    <div className="landing-page">
      <header className="landing-nav">
        ...
        <div className="landing-auth-actions">
          {user ? (
            <>
              <span className="logged-user">Hi, {user.username}</span>
              <button className="nav-launch" onClick={onCreateRoom}>Create room <span>↗</span></button>
              <button className="nav-signin" onClick={onLogout}>Logout</button>
            </>
          ) : (
            <>
      ...
      <main>
        <section className="hero" id="home">
          ...
          <div className="hero-copy">
            ...
            <p>A focused workspace where teams can sketch ideas, write code, and collaborate in real time — all in one room.</p>
            <div className="hero-actions">
              <button className="primary-cta" onClick={onCreateRoom}>Create a room <span>→</span></button>
              <form className="join-form" onSubmit={joinRoom}>
                <input value={roomId} onChange={(e) => setRoomId(e.target.value)} placeholder="Enter room code" aria-label="Room code" />
                <button type="submit">Join</button>
              </form>
            </div>
            <div className="hero-auth-note">
              {user ? <>Signed in as <strong>{user.username}</strong>. You can create a protected room.</> : <>Already have an account? <button type="button" onClick={onSignIn}>Sign in</button></>}
            </div>
            <div className="hero-note"><span className="mini-avatar">{user?.username?.[0]?.toUpperCase() || "C"}</span><span>Open a room and collaborate with your team.</span></div>
          </div>
          <div className="hero-product" aria-label="SyncSpace workspace preview">
            <div className="preview-window">
              <div className="preview-topbar"><div className="preview-dots"><i></i><i></i><i></i></div><span>SyncSpace / room-7f3a2c</span><div className="preview-live"><b></b> Live</div></div>
              <div className="preview-body">
                <div className="preview-whiteboard"><div className="preview-label"><span>✦</span> WHITEBOARD</div><div className="preview-canvas"><span className="sketch sketch-a"></span><span className="sketch sketch-b"></span><span className="sketch sketch-c"></span><span className="sketch-line"></span></div></div>
                <div className="preview-code"><div className="preview-code-head"><span>CODE</span><small>javascript</small></div><pre><code>{`01  const room = createRoom();
02
03  room.on("connect", () => {
04    collaborate();
05  });
06
07  export default room;`}</code></pre><div className="preview-output"><span>OUTPUT</span><b>Ready to run</b></div></div>
              </div>
            </div>
            <div className="floating-chip chip-live">● 3 collaborators</div><div className="floating-chip chip-sync">↗ Changes synced</div>
          </div>
        </section>
        ...
        <section className="about-section" id="about">
          <span className="section-kicker">03 / ABOUT SYNＣSPACE</span>
